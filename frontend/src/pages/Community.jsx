      ...
      <section className="home-container container mx-auto min-h-screen px-4 py-8 w-full">
        <div className="flex justify-between items-center">
          <div className="w-full" dir={language === 'he' ? 'rtl' : 'ltr'}>
            <h1 className={`title text-3xl font-bold text-gray-100 ${language === 'he' ? 'text-right' : ''}`}>
              {texts[language].title}
            </h1>
            <p
              className={`description text-lg text-gray-400 px-1 max-w-3xl pb-8 pt-3 ${language === 'he' ? 'text-right' : ''}`}
        ...
        <div className="card-section grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4">
          {loading ? (
